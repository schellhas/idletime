...
Verified current state (2026-04-06)
-----------------------------------
...
* each user gets a default ``root`` category automatically, and that category cannot be deleted
...
* the React frontend in ``../frontend/app`` can display that recommendation and perform a stateless skip
...
* ``categories`` has a ``user_id`` foreign key
* auth sessions resolve the current user from an ``HttpOnly`` cookie
* unverified users cannot log in
* each user gets a default ``root`` category as the fallback bucket for uncategorized activities
...
* ``exclude_activity_id`` enables a stateless skip for v1
...
1. broader automated tests for auth success and failure cases, session expiry, deeper user isolation coverage, and recommendation edge cases
...
3. frontend polish and UX improvements on top of the current React scaffold in ``../frontend/app``
4. a real email provider for non-development environments
5. charts, timer workflows, and continued API cleanup as the app surface expands

Recommended next implementation order
------------------------------------

1. expand tests for auth flows, session expiry, broader user isolation, and recommendation edge cases
2. refine the recommendation logic with available-time awareness and better alternative suggestions
3. polish the React frontend with better UX and richer timer/chart workflows on top of the current scaffold
...
Status: the current milestone is functionally in place and locally verified; the next push should focus on recommendation refinement, more tests, and frontend polish.
